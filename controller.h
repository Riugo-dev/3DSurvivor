...
//継続入力の間のフレーム数
#define CONTINUOUS_FRAMECOUNT (15)
...
private:
    ...
    //回転用変数
    //LeftStick（左ステック）
     float CurrentStickLocationLeft; //現在のスティック位置
     float PreviousStickLocationLeft; //前回のステック位置

    //RightStick（右ステック）
     float CurrentStickLocationRight; //現在のスティック位置
     float PreviousStickLocationRight; //前回のステック位置

     int m_ContinuousRightFrame;
     int m_ContinuousLeftFrame;

     bool m_IsRightStickUp;
     bool m_IsRightStickDown;
     bool m_IsRightStickLeft;
     bool m_IsRightStickRight;
     bool m_IsLeftStickUp;
     bool m_IsLeftStickDown;
     bool m_IsLeftStickLeft;
     bool m_IsLeftStickRight;
...
public:
    ...
    //コンストラクタ＆デストラクタ
    Controller();// コントローラーの初期化と接続台数の登録
    ~Controller();
    ...
    //ステックの入力受け取り
     ...
     bool LeftStickIsUpContinuous();        //左ステックの上継続入力
     bool LeftStickIsDownContinuous();      //左ステックの下継続入力
     bool LeftStickIsLeftContinuous();      //左ステックの左継続入力
     bool LeftStickIsRightContinuous();     //左ステックの右継続入力
     bool RightStickIsUpContinuous();       //右ステックの上継続入力
     bool RightStickIsDownContinuous();     //右ステックの下継続入力
     bool RightStickIsLeftContinuous();     //右ステックの左継続入力
     bool RightStickIsRightContinuous();    //右ステックの右継続入力
